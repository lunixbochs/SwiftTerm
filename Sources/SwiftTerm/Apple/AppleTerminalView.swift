...
//
    ...
    extension TerminalView {
        ...
        public func colorChanged(source _: Terminal, idx: Int?) {
            if let index = idx {
                colors[index] = nil
            } else {
                colors = Array(repeating: nil, count: 256)
        ...
        public func setBackgroundColor(source _: Terminal, color: Color) {
        ...
        public
        func setForegroundColor(source _: Terminal, color: Color) {
        ...
        func drawTerminalContents(dirtyRect _: TTRect, context: CGContext) {
            let lineDescent = CTFontGetDescent(fontSet.normal)
            let lineLeading = CTFontGetLeading(fontSet.normal)
            ...
            for row in terminal.buffer.yDisp ..< terminal.rows + terminal.buffer.yDisp {
                ...
                for run in CTLineGetGlyphRuns(ctline) as? [CTRun] ?? [] {
                    ...
                    var positions = runGlyphs.enumerated().map { (i: Int, _: CGGlyph) -> CGPoint in
        ...
        open func showCursor(source _: Terminal) {
            if caretView.superview == nil {
                addSubview(caretView)
            }
        }

        open func hideCursor(source _: Terminal) {
            caretView.removeFromSuperview()
        }

        open func bell(source _: Terminal) {
