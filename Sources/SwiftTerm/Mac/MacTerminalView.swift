...
//
    ...
    open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations {
        struct FontSet {
            ...
            public init(font baseFont: NSFont, fontSize _: CGFloat? = nil) {
                normal = baseFont
        ...
        var accessibility = AccessibilityService()
        ...
        public init(frame: CGRect, font: NSFont?) {
            fontSet = FontSet(font: font ?? FontSet.defaultFont)
        ...
        open func bufferActivated(source _: Terminal) {
            updateScroller()
        }

        open func send(source _: Terminal, data: ArraySlice<UInt8>) {
        ...
        open func getOptimalFrameSize() -> NSRect {
            NSRect(x: 0, y: 0, width: cellDimension.width * CGFloat(terminal.cols) + scroller.frame.width, height: cellDimension.height * CGFloat(terminal.rows))
        }

        func getEffectiveWidth(rect: NSRect) -> CGFloat {
            (rect.width - scroller.frame.width)
        }

        open func scrolled(source _: Terminal, yDisp _: Int) {
            // selectionView.notifyScrolled(source: terminal)
            updateScroller()
            terminalDelegate?.scrolled(source: self, position: scrollPosition)
        }

        open func linefeed(source _: Terminal) {
        ...
        // NSTextInputClient protocol implementation
        open func setMarkedText(_: Any, selectedRange _: NSRange, replacementRange _: NSRange) {
            // nothing
        }
        ...
        // NSTextInputClient protocol implementation
        open func attributedSubstring(forProposedRange _: NSRange, actualRange _: NSRangePointer?) -> NSAttributedString? {
        ...
        func cut(sender _: Any?) {}

        @objc
        open func paste(_: Any) {
        ...
        func zoomIn(sender _: Any) {}
        func zoomOut(sender _: Any) {}
        func zoomReset(sender _: Any) {}
        ...
        private func scrollingTimerElapsed(source _: Timer) {
    ...
    extension TerminalView: TerminalDelegate {
        open func isProcessTrusted(source _: Terminal) -> Bool {
            true
        }
        ...
        open func setTerminalTitle(source _: Terminal, title: String) {
        ...
        open func setTerminalIconTitle(source _: Terminal, title _: String) {
            //
        }

        // Terminal.Delegate method implementation
        open func windowCommand(source _: Terminal, command _: Terminal.WindowManipulationCommand) -> [UInt8]? {
            nil
        }
    }
