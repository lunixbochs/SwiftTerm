...
class TransitionTable {
    ...
    init(len: Int) {
        table = Array(repeating: 0, count: len)
    }
...
class EscapeSequenceParser {
    ...
    typealias CsiHandler = ([Int], cstring) -> Void
    typealias CsiHandlerFallback = ([Int], cstring, UInt8) -> Void

    // String with payload
    typealias OscHandler = (ArraySlice<UInt8>) -> Void
    typealias OscHandlerFallback = (Int) -> Void

    // Collect + flag
    typealias EscHandler = (cstring, UInt8) -> Void
    typealias EscHandlerFallback = (cstring, UInt8) -> Void

    // Range of bytes to print out
    typealias PrintHandler = (ArraySlice<UInt8>) -> Void

    typealias ExecuteHandler = () -> Void
    ...
    var activeDcsHandler: DcsHandler?
    var errorHandler: (ParsingState) -> ParsingState = { (state: ParsingState) -> ParsingState in state }
    ...
    var printHandler: PrintHandler = { (_: ArraySlice<UInt8>) -> Void in }
    var printStateReset: () -> Void = {}
    ...
    func ParserEscHandlerFallback(collect _: cstring, flag _: UInt8) {}

    var escHandlerFallback: EscHandlerFallback = { (_: cstring, _: UInt8) in
    }
    ...
    var executeHandlerFallback: ExecuteHandler = { () -> Void in
    }

    var csiHandlerFallback: CsiHandlerFallback = { (_: [Int], _: cstring, code: UInt8) -> Void in
        print("Cannot handle ESC-\(code)")
    }

    var oscHandlerFallback: OscHandlerFallback = { (_: Int) -> Void in
    }
