...
open class Terminal {
    ...
    public private(set) var hostCurrentDirectory: String?
    ...
    public private(set) var hostCurrentDocument: String?
    ...
    public var foregroundColor = Color.defaultForeground {
        didSet {
            if settingFgColor {
                return
            }
    ...
    public var backgroundColor = Color.defaultBackground {
        didSet {
            if settingBgColor {
                return
            }
    ...
    public func getCharacter(col: Int, row: Int) -> Character? {
        getCharData(col: col, row: row)?.getCharacter()
    }
    ...
    class DECRQSS: DcsHandler {
        ...
        func hook(collect _: cstring, parameters _: [Int], flag _: UInt8) {
            data = []
        }
    ...
    func configureParser(_ parser: EscapeSequenceParser) {
        parser.csiHandlerFallback = { (pars: [Int], collect: cstring, code: UInt8) -> Void in
        ...
        // ESC handlers
        //
        parser.setEscHandler("6") { _, _ in self.columnIndex(back: true) }
        parser.setEscHandler("7") { _, _ in self.cmdSaveCursor([], []) }
        parser.setEscHandler("8") { _, _ in self.cmdRestoreCursor([], []) }
        parser.setEscHandler("9") { _, _ in self.columnIndex(back: false) }
        parser.setEscHandler("D") { _, _ in self.cmdIndex() }
        parser.setEscHandler("E") { _, _ in self.cmdNextLine() }
        parser.setEscHandler("H") { _, _ in self.cmdTabSet() }
        parser.setEscHandler("M") { _, _ in self.reverseIndex() }
        parser.setEscHandler("=") { _, _ in self.cmdKeypadApplicationMode() }
        parser.setEscHandler(">") { _, _ in self.cmdKeypadNumericMode() }
        parser.setEscHandler("c") { _, _ in self.cmdReset() }
        parser.setEscHandler("n") { _, _ in self.setgLevel(2) }
        parser.setEscHandler("o") { _, _ in self.setgLevel(3) }
        parser.setEscHandler("|") { _, _ in self.setgLevel(3) }
        parser.setEscHandler("}") { _, _ in self.setgLevel(2) }
        parser.setEscHandler("~") { _, _ in self.setgLevel(1) }
        parser.setEscHandler("%@") { _, _ in self.cmdSelectDefaultCharset() }
        parser.setEscHandler("%G") { _, _ in self.cmdSelectDefaultCharset() }
        parser.setEscHandler("#8") { _, _ in self.cmdScreenAlignmentPattern() }
        parser.setEscHandler(" G") { _, _ in self.cmdSet8BitControls() }
        parser.setEscHandler(" F") { _, _ in self.cmdSet7BitControls() }
        for bflag in CharSets.all.keys {
            let flag = String(UnicodeScalar(bflag))

            parser.setEscHandler("(" + flag) { _, f in self.selectCharset([UInt8(ascii: "(")] + [f]) }
            parser.setEscHandler(")" + flag) { _, f in self.selectCharset([UInt8(ascii: ")")] + [f]) }
            parser.setEscHandler("*" + flag) { _, f in self.selectCharset([UInt8(ascii: "*")] + [f]) }
            parser.setEscHandler("+" + flag) { _, f in self.selectCharset([UInt8(ascii: "+")] + [f]) }
            parser.setEscHandler("-" + flag) { _, f in self.selectCharset([UInt8(ascii: "-")] + [f]) }
    ...
    var lastBufferStorage: (buffer: Buffer, y: Int, x: Int, cols: Int, rows: Int)?
    ...
    var hyperLinkTracking: (start: Position, payload: String)?
    ...
    //
    func cmdInsertChars(_ pars: [Int], _: cstring) {
    ...
    //
    func cmdCursorUp(_ pars: [Int], _: cstring) {
    ...
    //
    func cmdCursorDown(_ pars: [Int], _: cstring) {
        let buffer = self.buffer
        let param = max(pars.count > 0 ? pars[0] : 1, 1)
    ...
    //
    func cmdCursorForward(_ pars: [Int], _: cstring) {
    ...
    //
    func cmdCursorBackward(_ pars: [Int], _: cstring) {
    ...
    //
    func cmdCursorForwardTab(_ pars: [Int], _: cstring) {
    ...
    //
    func cmdCursorPosition(_ pars: [Int], _: cstring) {
    ...
    //
    func cmdCursorCharAbsolute(_ pars: [Int], _: cstring) {
        let buffer = self.buffer
        let param = max(pars.count > 0 ? pars[0] : 1, 1)
    ...
    //
    func cmdEraseInLine(_ pars: [Int], _: cstring) {
    ...
    //     Ps = 2  -> Selective Erase All.
    //
    func cmdEraseInDisplay(_ pars: [Int], _: cstring) {
        let p = pars.count == 0 ? 0 : pars[0]
        var j: Int
        switch p {
        case 0:
    ...
    //
    func cmdInsertLines(_ pars: [Int], _: cstring) {
        let buffer = self.buffer
    ...
    func cmdRestoreCursor(_: [Int], _: cstring) {
    ...
    func cmdDECRQCRA(_ pars: [Int], _: cstring) {
        var checksum: UInt32 = 0
        let rid = pars.count > 0 ? pars[0] : 1
        _ = pars.count > 1 ? pars[1] : 0
        var result = "0000"
    ...
    func cmdSetMargins(_ pars: [Int], _: cstring) {
    ...
    //
    func cmdSaveCursor(_: [Int], _: cstring) {
    ...
    func cmdSetConformanceLevel(_ pars: [Int], _: cstring) {
        if pars.count > 0 {
            let level = pars[0]
            switch level {
            case 61:
    ...
    //     Ps.
    //
    func cmdCharAttributes(_ pars: [Int], _: cstring) {
    ...
    //
    func cmdTabClear(_ pars: [Int], _: cstring) {
        let p = pars.count == 0 ? 0 : pars[0]
        if p == 0 {
    ...
    //   [1,1]) (HVP).
    //
    func cmdHVPosition(_ pars: [Int], _: cstring) {
        var p = 1
        var q = 1
        if pars.count > 0 {
            p = max(pars[0], 1)
    ...
    // reuse CSI Ps B ?
    //
    func cmdVPositionRelative(_ pars: [Int], _: cstring) {
        let p = max(pars.count == 0 ? 1 : pars[0], 1)
        let newY = buffer.y + p
    ...
    //
    func cmdLinePosAbsolute(_ pars: [Int], collect _: cstring) {
        let p = max(pars.count == 0 ? 1 : pars[0], 1)
    ...
    //
    func cmdRepeatPrecedingCharacter(_ pars: [Int], collect _: cstring) {
        let p = max(pars.count == 0 ? 1 : pars[0], 1)
    ...
    // reuse CSI Ps C ?
    //
    func cmdHPositionRelative(_ pars: [Int], collect _: cstring) {
        let p = max(pars.count == 0 ? 1 : pars[0], 1)
    ...
    //
    func cmdCharPosAbsolute(_ pars: [Int], collect _: cstring) {
        let p = max(pars.count == 0 ? 1 : pars[0], 1)
    ...
    //
    func cmdCursorBackwardTab(_ pars: [Int], collect _: cstring) {
        if buffer.x > cols {
            return
        }
        let p = max(pars.count == 0 ? 1 : pars[0], 1)
    ...
    //
    func cmdEraseChars(_ pars: [Int], collect _: cstring) {
        let p = max(pars.count == 0 ? 1 : pars[0], 1)
    ...
    //
    func cmdScrollUp(_ pars: [Int], collect _: cstring) {
        let p = max(pars.count == 0 ? 1 : pars[0], 1)
        let da = CharData.defaultAttr
    ...
    //
    func cmdDeleteChars(pars: [Int], _: cstring) {
        let buffer = self.buffer
        var p = max(pars.count == 0 ? 1 : pars[0], 1)
    ...
    //
    func cmdDeleteLines(_ pars: [Int], _: cstring) {
        restrictCursor()
        let buffer = self.buffer
    ...
    func cmdDeleteColumns(_ pars: [Int], _: cstring) {
        let buffer = self.buffer
    ...
    var blankLine = BufferLine(cols: 0)
    ...
    func matchColor(_: Int, _: Int, _: Int) -> Int32 {
        // TODO:
        abort()
    }
...
public extension TerminalDelegate {
    func cursorStyleChanged(source _: Terminal, newStyle _: CursorStyle) {
        // Do nothing
    }
    ...
    func linefeed(source _: Terminal) {
        // nothing
    }

    func bufferActivated(source _: Terminal) {
        // nothing
    }

    func windowCommand(source _: Terminal, command _: Terminal.WindowManipulationCommand) -> [UInt8]? {
        // no special handling
        nil
    }
    ...
    func bell(source _: Terminal) {
        // nothing
    }

    func isProcessTrusted(source _: Terminal) -> Bool {
        true
    }

    func selectionChanged(source _: Terminal) {
        // nothing
    }
    ...
    func iTermContent(source _: Terminal, _: String) {}
}
