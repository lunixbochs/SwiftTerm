...
class Buffer {
    ...
    var lines: CircularList<BufferLine> { _lines }
    ...
    func makeEmptyLine(_: Int) -> BufferLine {
    ...
    func clearTabStops() {
        tabStops = Array(repeating: false, count: tabStops.count)
    }
    ...
    func getWrappedLineTrimmedLength(_ lines: CircularList<BufferLine>, _ row: Int, _ cols: Int) -> Int {
        getWrappedLineTrimmedLength(lines[row], row == lines.count - 1 ? nil : lines[row + 1], cols)
    }
    ...
    func reflowWider(_ oldCols: Int, _: Int, _ newCols: Int, _ newRows: Int) {
    ...
    func reflowNarrower(_ oldCols: Int, _: Int, _ newCols: Int, _ newRows: Int) {
    ...
    func dump() {
        ...
        do {
            try str.write(to: URL(fileURLWithPath: file), atomically: false, encoding: .utf8)
