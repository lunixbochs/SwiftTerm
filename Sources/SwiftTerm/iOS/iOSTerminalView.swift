    ...
    open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate {
        struct FontSet {
            ...
            public init(font baseFont: UIFont) {
                normal = baseFont
                bold = UIFont(descriptor: baseFont.fontDescriptor.withSymbolicTraits([.traitBold])!, size: 0)
        ...
        open func getOptimalFrameSize() -> CGRect {
            CGRect(x: 0, y: 0, width: cellDimension.width * CGFloat(terminal.cols), height: cellDimension.height * CGFloat(terminal.rows))
        }
        ...
        open func insertText(_ text: String) {
            if terminalAccessory?.controlModifier ?? false {
                send(applyControlToEventCharacters(text))
                terminalAccessory?.controlModifier = false
            } else {
                send(txt: text)
            }
        ...
        override public func pressesBegan(_ presses: Set<UIPress>, with _: UIPressesEvent?) {
...
//
    ...
    extension TerminalView: TerminalDelegate {
        ...
        open func mouseModeChanged(source _: Terminal) {
            // iOS TODO
            // X
        }
    ...
    extension UIColor {
        ...
        static func make(red: CGFloat, green: CGFloat, blue: CGFloat, alpha _: CGFloat) -> TTColor {
            UIColor(red: red,
                    green: green,
                    blue: blue,
                    alpha: 1.0)
