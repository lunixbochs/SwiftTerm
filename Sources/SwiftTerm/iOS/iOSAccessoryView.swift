...
//
    ...
    public class TerminalAccessory: UIInputView, UIInputViewAudioFeedback {
        ...
        @objc func esc(_: AnyObject) { clickAndSend([0x1B]) }
        @objc func tab(_: AnyObject) { clickAndSend([0x9]) }
        @objc func tilde(_: AnyObject) { clickAndSend([UInt8(ascii: "~")]) }
        @objc func pipe(_: AnyObject) { clickAndSend([UInt8(ascii: "|")]) }
        @objc func slash(_: AnyObject) { clickAndSend([UInt8(ascii: "/")]) }
        @objc func dash(_: AnyObject) { clickAndSend([UInt8(ascii: "-")]) }

        @objc
        func ctrl(_: UIButton) {
            controlModifier.toggle()
        }

        // Controls the timer for auto-repeat
        var repeatCommand: (() -> Void)?
        var repeatTimer: Timer?

        func startTimerForKeypress(repeatKey: @escaping () -> Void) {
            repeatKey()
